...
int main(int argc, char* argv[]){
    cout << "=======================================ART======================================"<<endl;
    cout << "                            ART_Illumina (2008-2014)                            "<<endl;
    cout << "                            Q Version 2.1.8 (Mar 8, 2014)                      "<<endl;
    cout << "                     Contact: Weichun Huang at whduke@gmail.com                 "<<endl; 
    cout << "================================================================================"<<endl<<endl;
    ...
    if(!arg_success){
        ...
        cout << "art_illumina [options] -sam -i <seq_ref_file> -l <read_length> -f <fold_coverage> -m <mean_fragsize> -s <std_fragsize> -o <outfile_prefix>"<<endl<<endl;
        ...
        cout << "* For single-end simulation, ART requires input sequence file, outputfile prefix, read length and fold coverage." << endl << endl;;
        ...
        cout << " 	art_illumina -sam -i reference.fa -l 50 -f 10 -o single_dat" <<endl<<endl;
        cout << " 2) paired-end read simulation" <<endl;
        cout << "       art_illumina -sam -i reference.fa -p -l 50 -f 20 -m 200 -s 10 -o paired_dat" <<endl<<endl;
    ...
    if(is_pairend_read){
        ...
        while(seq_reader.next_seq(id,a_art.ref_seq)){ 
            ...
            while(t_num_read>0){
                ...
                if(is_matepair){
                    if(amplicon) a_art.next_matepair_ampread_indel_cmp(a_read, a_read_2);
                    else a_art.next_pair_read_indel_mate(a_read, a_read_2); 
                }
                else{
                    if(amplicon) a_art.next_pair_ampread_indel_cmp(a_read, a_read_2); 
                    else a_art.next_pair_read_indel_cmp(a_read, a_read_2); 
                }
                if(mask_n){ 
