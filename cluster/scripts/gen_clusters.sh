...
echo $PATH
samtools view -b -h -S AC217266.sam | samtools sort - AC217266_sorted
...
samtools view all_for_aligned.bam | gen_sam_clusters.py > orig_cluster.txt
samtools view all_rev_aligned.bam | gen_sam_clusters.py >> orig_cluster.txt
samtools view all_not_aligned.bam | cut -f 1 >> orig_cluster.txt
